...
public class PeakFinderCommandTest {
	...
	public Dataset simulateDataset() {
		...
		Dataset dataset = datasetService.create(dim, "simulated Image", axes, 16, false, false);

		for (int t=0; t < dim[2]; t++)
			for (int x=0; x < dim[0]; x++)
				 for (int y=0; y < dim[1]; y++) {
					Gaussian2D peak1 = new Gaussian2D(1000d, 3000d, 10d, 10d + t/4, 1.2d);
				 	Gaussian2D peak2 = new Gaussian2D(1000d, 3000d, 32.5d, 40d + t/4, 1.2d);
				 	Gaussian2D peak3 = new Gaussian2D(1000d, 3000d, 43.7d, 26.7d + t/4, 1.2d);
					...
					dataset.getImgPlus().randomAccess().setPositionAndGet(x,y,t).setReal(r.nextInt(1000) + 500 
						+ peak1.getValue(x, y)
						+ peak2.getValue(x, y)
						+ peak3.getValue(x, y)
						+ peak4.getValue(x, y)
